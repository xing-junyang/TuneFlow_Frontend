...
const showExpandButton = computed(() => {
  return albumInfo.value === undefined ? "" : albumInfo.value.description.length > limit;
});
...
const getSongListDetail = async () => {
  ...
  const sig = await getAlbum(Number(songlistId))
    .then((res) => {
  ...
  await getAlbumAllSongs(Number(songlistId))
    .then((res) => {
  ...
  <div class="song-list-detail-main-container">
    ...
    <div class="album-header">
      ...
      <div class="album-info">
        ...
        <p
          ...
          v-html="shortenedDescription"
        ></p>
        <button class="expand-button" v-if="showExpandButton" @click="toggleCollapse">
          {{ isCollapsed ? "展开" : "收起" }}
        </button>
        <div class="album-meta">
          <span class="rating" v-if="albumInfo && albumInfo.mark">★ {{ albumInfo.mark.toFixed(1) }}</span>
          <span class="separator" v-if="albumInfo && albumInfo.mark">•</span>
          ...
          <span class="songs-count"
            >{{ albumInfo ? albumInfo.songsId.length : 0 }} 首歌曲</span
          >
        </div>
      </div>
    </div>
    ...
    <div class="controls">
      ...
      <button
        ...
        @click="openCommentModal" :disabled="isLoading">
				<i class="fa-solid fa-comment"></i>
				查看歌单评论
			</button>
			<button class="edit-song-list-btn" @click="addSongModalVisible = true"
        :disabled="isLoading"
        v-if="isAdmin"
      >
        添加歌曲
      ...
      <button
        class="edit-song-list-btn"
        @click="editSongListModalVisible = true"
        :disabled="isLoading"
        v-if="isAdmin"
      >
        编辑歌单
        ...
        <tbody>
          <tr v-for="(song, index) in songs" :key="song.id" class="song-row">
            ...
            <td
              v-if="isAdmin"
              class="add-to-playlist-btn"
              @click="deleteFromPlayList(index)"
            >
              <!--						add to playlist button-->
              <i class="fa-solid fa-delete-left"></i>
            </td>
    ...
    <!-- 添加歌曲弹窗 -->
    <AddSongToSongList
      v-if="addSongModalVisible"
      @closeAddSongToSongList="addSongModalVisible = false"
      :songListId="Number(albumInfo.id)"
    />

    <!-- 删除歌曲弹窗 -->
    <DeleteSongFromSongList
      v-if="deleteSongModalVisible"
      @closeDeleteSongFromSongList="deleteSongModalVisible = false"
      :songListId="Number(albumInfo.id)"
      :songId="deleteSongId"
    ...
    <!-- 编辑歌单弹窗 -->
    <EditSongList
      v-if="editSongListModalVisible"
      @closeEditSongList="editSongListModalVisible = false"
      :songListId="Number(albumInfo.id)"
    />

    <!-- 编辑歌曲弹窗 -->
    <EditSong
      v-if="editSongModalVisible"
      @closeEditSong="editSongModalVisible = false"
      :songId="editSongId"
    />
